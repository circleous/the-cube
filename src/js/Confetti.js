...
class ConfettiStage extends Animation {
  ...
  resizeViewport() {
    ...
    this.height =
      2 * Math.tan(fovRad / 2) * (this.game.world.camera.position.length() - this.distanceFromCube);
...
class Particle {
  ...
  reset(randomHeight = true) {
    ...
    this.revolutionSpeed = THREE.MathUtils.randFloat(
  ...
  update(delta) {
    ...
    if (this.mesh.position.y < -this.confetti.height - this.size) {
      if (this.confetti.playing) this.reset(false);
      else this.stop();
    }
  }
}
